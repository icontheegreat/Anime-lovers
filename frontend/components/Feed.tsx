...
export default function Feed({ initialPost }: { initialPost?: Post }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPosts((currentPosts) => {
        const seen = new Set(currentPosts.map((post) => post._id));
  ...
  useEffect(() => {
    if (posts.length - index <= 3 && cursor) {
      load();
    }
  }, [posts.length, index, cursor, load]);
  ...
  const move = (dir: number) => {
    ...
    if (next >= 0 && next < posts.length) {
      setIndex(next);
      x.set(0);
    } else if (next >= posts.length && cursor) {
      load();
    }
  };
